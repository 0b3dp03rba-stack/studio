  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center space-y-8">
        <div className="w-24 h-24 rounded-none bg-primary/10 flex items-center justify-center text-primary glow-primary animate-bounce">
          <Ghost size={48} />
        </div>
        ...
        <Button asChild className="h-14 px-10 neon-gradient text-background font-black rounded-none uppercase text-[10px] tracking-[0.2em] shadow-2xl">
    ...
    >
      <div className="max-w-md mx-auto space-y-8 animate-in relative z-10 p-6 pb-24">
        <div className="flex justify-end">
          <Button variant="ghost" size="icon" onClick={() => { navigator.clipboard.writeText(window.location.href); toast({title: "Tersalin"}); }} className="w-12 h-12 rounded-none glass-card text-white">
            <Share2 size={20} />
          </Button>
        </div>
        ...
        <div className="text-center space-y-6">
          <div 
            className="mx-auto w-32 h-32 rounded-none p-1 shadow-2xl animate-flowing-gradient"
            style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%', boxShadow: `0 0 50px -10px ${primaryColor}99` }}
          >
            <div className="w-full h-full rounded-none bg-background flex items-center justify-center overflow-hidden border-4 border-background">
              ...
              {profile.socialLinks.map((social: any, i: number) => {
                ...
                return (
                  <button 
                    ...
                    className="w-12 h-12 rounded-none glass-card flex items-center justify-center text-white/60 hover:text-white transition-all hover:scale-110 border border-white/5 active:scale-95"
                  >
        ...
        <div className="relative group">
          <div className="absolute inset-0 bg-primary/20 blur-xl opacity-0 group-focus-within:opacity-100 transition-opacity rounded-none" />
          <div className="relative glass-card rounded-none flex items-center px-4 gap-3 border border-white/10 h-14">
        ...
        <div className="space-y-6">
          {!searchQuery && newestLink && (
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full p-1 rounded-none hover:scale-[1.02] transition-transform shadow-2xl animate-flowing-gradient relative overflow-hidden"
                style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%' }}
              >
                <div className="absolute top-0 right-0 p-2">
                   <div className="bg-white text-background text-[8px] font-black px-2 py-0.5 rounded-none shadow-lg">NEW</div>
                </div>
                <div className="w-full h-24 bg-black/80 backdrop-blur-2xl rounded-none flex items-center px-6 gap-4 border border-white/10">
                  <div className="w-16 h-16 rounded-none bg-white/5 flex items-center justify-center overflow-hidden border border-white/10 shadow-xl">
          ...
          <div className="space-y-4 pt-4 border-t border-white/5">
            ...
            {filteredGroups.map(group => (
              <Link key={group.id} href={`/${username}/g/${group.id}`} className="block">
                <div 
                  className="p-0.5 rounded-none animate-flowing-gradient transition-transform active:scale-95 shadow-xl"
                  style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%' }}
                >
                  <div className="w-full h-24 bg-black/70 backdrop-blur-2xl rounded-none flex items-center px-6 gap-4 border border-white/10 relative overflow-hidden">
                    <div className="w-16 h-16 rounded-none bg-white/5 flex items-center justify-center overflow-hidden border border-white/10 shadow-xl shrink-0">
          ...
          <div className="space-y-4 pt-4 border-t border-white/5">
            ...
            {filteredStandaloneLinks.map(link => (
              <button
                ...
                className="w-full p-0.5 rounded-none hover:scale-[1.02] transition-transform shadow-xl animate-flowing-gradient"
                style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%' }}
              >
                <div className="w-full h-20 bg-black/80 backdrop-blur-xl rounded-none flex items-center px-6 gap-4 border border-white/10">
                  <div className="w-12 h-12 rounded-none bg-white/5 flex items-center justify-center overflow-hidden border border-white/5">
          ...
          {searchQuery && filteredGroupedLinks.map(link => (
             <button
              ...
              className="w-full p-0.5 rounded-none hover:scale-[1.02] transition-transform shadow-xl"
          ...
          {searchQuery && filteredGroups.length === 0 && filteredStandaloneLinks.length === 0 && filteredGroupedLinks.length === 0 && (
             <div className="text-center py-20 opacity-20 font-black uppercase text-xs tracking-widest text-white">
        ...
        <div className="pt-12 text-center">
          <Button asChild className="w-full h-14 neon-gradient text-background font-black rounded-none glow-primary text-[10px] uppercase tracking-widest shadow-2xl transition-all animate-flowing-gradient" style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%' }}>
      ...
      <Dialog open={!!selectedSocial} onOpenChange={(open) => !open && setSelectedSocial(null)}>
        <DialogContent className="glass-card border-none rounded-none bg-background/95 backdrop-blur-3xl p-8 shadow-2xl max-w-[90%] sm:max-w-sm mx-auto overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1.5 neon-gradient" style={{ backgroundImage: dynamicGradient }} />
          <div className="flex flex-col items-center text-center space-y-6 py-4">
            <div 
              className="w-24 h-24 rounded-none p-1 shadow-2xl animate-flowing-gradient"
              style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%' }}
            >
              <div className="w-full h-full rounded-none bg-background flex items-center justify-center overflow-hidden border-4 border-background">
                {selectedSocial && (() => {
            ...
            <Button 
              asChild 
              className="w-full h-14 neon-gradient text-background font-black rounded-none glow-primary text-[10px] uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all"
              style={{ backgroundImage: dynamicGradient, backgroundSize: '200% 200%' }}
            >
              <a 
